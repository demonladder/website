        ...
        <div className='grid grid-cols-12 max-xl:gap-2'>
            ...
            <div className='col-span-11 flex grow gap-2 lg:gap-6 bg-theme-700'>
                ...
                <div className='self-center text-sm lg:text-xl'>
                    <Link to={'/level/' + level.LevelID} className='font-bold underline break-all whitespace-pre-wrap'>
                        {level.Level.Meta.Name}
                    </Link>
                    <p>
                        <i>{level.Level.Meta.Publisher.name}</i>
                    </p>
                    <IDButton id={level.LevelID} />
                </div>
                <div
                    className={`ms-auto w-10 lg:w-32 lg:h-32 flex flex-col justify-center group enj-${roundedEnjoyment}`}
                >
                    <p className='text-center'>Enjoyment</p>
                    <p className='text-5xl text-center group-hover:hidden '>
                        {level.Level.Enjoyment?.toFixed() ?? 'N/A'}
                    </p>
                    <p className='text-5xl text-center hidden group-hover:block '>
                        {level.Level.Enjoyment?.toFixed(2) ?? 'N/A'}
                    </p>
                </div>
            </div>
        </div>
