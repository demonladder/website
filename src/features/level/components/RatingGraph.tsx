...
export default function RatingGraph({ levelMeta, twoPlayer, setShowTwoPlayerStats }: Props) {
    ...
    const filledRatingData = useMemo(() => {
        ...
        const highestRating = ratingData.reduce(
            (acc, cur) => Math.max(acc, cur.Rating),
            ratingData.at(0)?.Rating ?? MAX_TIER,
        );
    ...
    const filledEnjoymentData = useMemo(() => {
        ...
        const lowestEnjoyment = enjoymentData.reduce(
            (acc, cur) => Math.min(acc, cur.Enjoyment),
            enjoymentData.at(0)?.Enjoyment ?? 0,
        );
        const highestEnjoyment = enjoymentData.reduce(
            (acc, cur) => Math.max(acc, cur.Enjoyment),
            enjoymentData.at(0)?.Enjoyment ?? 10,
        );
    ...
    if (ratingData?.length === 0 && enjoymentData?.length === 0) return;
                                ...
                                <tbody>
                                    ...
                                    {filledRatingData.map((d) => (
                                        <tr key={d.Rating}>
                                            ...
                                            <td
                                                className='ps-4 pe-2 text-right cursor-pointer select-none'
