...
export function LeaderboardEntry({ sum, discordData, user, highestScore }: { sum: number; user: User; discordData: Pick<DiscordUserData, 'AccentColor'> | null; highestScore?: number; }) {
    ...
    return (
        <div className='mt-2 max-md:text-xs grid grid-cols-[auto_1fr_auto] gap-2'>
            <object data={`/api/user/${user.ID}/pfp?size=40`} type='image/png' className='rounded-full size-10' />
