...
function Statistic({ label, children }: { label: string; children: number | string }) {
    return (
        <p>{label} <span className='float-right'>{anyOrLoading(children)}</span></p>
    );
}
            ...
            {status === 'success' &&
                <div className='grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-2 gap-8'>
                    ...
                    <div>
                        ...
                        <Statistic  label='Coverage'>{((data.totalRatedLevels ?? 0) / (data.totalLevels.now ?? 1) * 100).toFixed(2)}</Statistic>
                    </div>
                    <div>
                        ...
                        <p>Oldest pending <span className='float-right'>{data.oldestQueuedSubmission ? ms(Date.now() - new Date(data.oldestQueuedSubmission.replace(' +00:00', 'Z').replace(' ', 'T')).getTime()) : '0s'} ago</span></p>
                    </div>
                    <div>
