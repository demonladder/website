...
export default function SortMenu({ set }: Props) {
    ...
    return (
        <div className='relative h-full z-[11]'>
            ...
            <div
                ref={resultsRef}
                className='absolute left-0 sm:left-1/2 sm:-translate-x-1/2 grid overflow-hidden transition-[grid-template-rows]'
                    ...
                    <div className='p-3 flex flex-col gap-2'>
                        ...
                        <div>
                            ...
                            <label htmlFor={Sorts.LEVEL_RATING} className='flex items-center gap-2 select-none'>
                                <RadioButton
                                    id={Sorts.LEVEL_RATING}
                                    name='sort'
                                    checked={sorter === Sorts.LEVEL_RATING}
                                    onChange={handleSortMenu}
                                />
                                Level rating
                            </label>
                            <label htmlFor={Sorts.ENJOYMENT} className='flex items-center gap-2 select-none'>
                                <RadioButton
                                    id={Sorts.ENJOYMENT}
                                    name='sort'
                                    checked={sorter === Sorts.ENJOYMENT}
                                    onChange={handleSortMenu}
                                />
                                Enjoyment
                            </label>
                            <label htmlFor={Sorts.LEVEL_ENJOYMENT} className='flex items-center gap-2 select-none'>
                                <RadioButton
                                    id={Sorts.LEVEL_ENJOYMENT}
                                    name='sort'
                                    checked={sorter === Sorts.LEVEL_ENJOYMENT}
                                    onChange={handleSortMenu}
                                />
                                Level enjoyment
                            </label>
