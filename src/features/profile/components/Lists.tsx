    ...
    return (
        <section className='mt-8'>
            <Heading2>
                ...
                {(lists?.length ?? 0) > 4 && (
                    <button
                        onClick={() => setMaxLists(lists?.length ?? 4)}
                        className='text-base float-right font-normal text-blue-500'
                    >
                        View all
                    </button>
                )}
            ...
            <div
                ...
                style={{
                    gridAutoRows: '120px auto auto auto',
...
function ListCard({ list }: { list: GetUserListsResponse }) {
    ...
    const contextMenu = useContextMenu([{ text: 'Delete', type: 'danger', userID: list.OwnerID }]);

    const secondsSinceCreation = (Date.now() - new Date(list.createdAt).getTime()) / 1000;
    const secondsSinceUpdate = (Date.now() - new Date(list.updatedAt).getTime()) / 1000;
