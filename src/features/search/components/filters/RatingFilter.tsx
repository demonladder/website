    ...
    return (
        <div className='col-span-12 sm:col-span-6 lg:col-span-4 xl:col-span-2'>
            ...
            <div className='flex items-center'>
                <NumberInput min='1' max={MAX_TIER} value={minRating ?? ''} onChange={(e) => setMinRating(parseNumber(e.target.value))} />
                <p className='m-0 mx-2'>to</p>
                <NumberInput min='1' max={MAX_TIER} value={maxRating ?? ''} onChange={(e) => setMaxRating(parseNumber(e.target.value))} />
            </div>
        </div>
    );
}
