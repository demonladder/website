    ...
    return (
        <div className='flex items-center gap-2'>
            <div className='relative whitespace-nowrap'>
                <TonalButton onClick={() => setShow((prev) => !prev)}>{`Sort by: ${sorts[sorter as Sorts]}`}</TonalButton>
                <ul className={'absolute z-50 py-2 rounded-lg min-w-28 max-w-60 bg-theme-600 transition-opacity ' + (show ? 'opacity-100': 'opacity-0 pointer-events-none')} ref={menuRef}>
                    {Object.entries(sorts).map(([key, value]) => (
                        <li key={key}>
                            <button className='h-12 px-3 text-start w-full hover:bg-theme-700 transition-colors' onClick={() => setSorter(key)}>{value}</button>
                        </li>
                    ))}
                </ul>
            </div>
