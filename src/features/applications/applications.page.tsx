    ...
    return (
        <Page title='Applications'>
            <div className='flex justify-between items-center'>
                ...
                <PrimaryButton size='md' onClick={() => openCreateModal()}>
                    New application
                </PrimaryButton>
            </div>
            {status === 'success' && (
                <div className='flex flex-wrap gap-4 mt-8'>
                    {data.map((app) => (
                        <Link
                            to={`${app.ID}`}
                            className='bg-theme-700 p-2 round:rounded-lg shadow outline outline-white/15'
                            key={app.ID}
                        >
                            <img
                                src={`/api/user/${app.botID}/pfp?size=128`}
                                width='128'
                                height='128'
