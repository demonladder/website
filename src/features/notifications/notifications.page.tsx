...
export default function Notifications() {
    ...
    function markAllRead() {
        ...
        const promise = MarkAllReadRequest()
            .then(() => {
    ...
    return (
        <Page>
            ...
            <div className='mb-4'>
                ...
                <SecondaryButton className='ms-4' onClick={markAllRead}>
                    Mark all as read
                </SecondaryButton>
            </div>
...
function Notification({ notif }: { notif: NotificationResponse }) {
    ...
    function onClick() {
        ...
        MarkReadNotifications([notif.ID])
            ?.then(() => {
                void queryClient.invalidateQueries({ queryKey: ['notifications'] });
            })
            .catch(() => {
    ...
    return (
        <li className='flex justify-between group'>
            <div>
                <p className='text-lg'>
                    {!notif.IsRead && <i className='bx bxs-circle' />} {notif.Message}{' '}
                    <span className='text-sm text-gray-400'>Sent at {notif.SentAt.toLocaleString()}</span>
                </p>
            </div>
            {!notif.IsRead && (
                <button className='xl:hidden group-hover:inline-block text-xl' onClick={onClick}>
                    <i className='bx bx-x' />
                </button>
            )}
        </li>
