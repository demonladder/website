...
export default function TierRoulette() {
    const [rouletteLevels, setRouletteLevels] = useLocalStorage<TierRouletteResponse[] | null>(
        'tierRouletteLevels',
        null,
    );
    ...
    const generateMutation = useMutation({
        ...
        onError: (error: AxiosError) =>
            void toast.error((error.response?.data as { error: string })?.error ?? 'An error occurred'),
        onSuccess: (data) => {
    ...
    return (
        <div>
            ...
            <p>
                Tier roulettes are a kind of roulette where the tier of the next level drops or increases over time. For
                example, the first 5 levels might be tier 30, the next 5 tier 29 and so on.
            </p>
            <p>
                Just like regular roulettes, you must get 1% more than the last level starting at 1% for level #1, 2%
                for level #2 and n% for level #n. Roulettes contain 100 levels, therefore you have to beat at least the
                very last level.
            </p>
            <div className='my-4'>
...
function List({
    ...
    levels?: TierRouletteResponse[] | null;
    progress: number;
            ...
            {levels.slice(0, progress + 1).map((l, i) => (
                <li className='even:bg-gray-700 py-4' key={l.ID}>
                    <div className='flex'>
                        ...
                        <p className={`w-20 text-center py-1 enj-${(l.Enjoyment ?? -1).toFixed()}`}>
