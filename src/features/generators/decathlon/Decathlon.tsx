...
export default function Decathlon() {
    ...
    const generateMutation = useMutation({
        mutationFn: () =>
            toast.promise(GenerateDecathlon, {
                pending: 'Generating...',
                success: 'Generated!',
    ...
    const reRollMutation = useMutation({
        mutationFn: () =>
            toast.promise(ReRollDecathlonLevel, {
                pending: 'Rolling...',
                success: 'Level re-rolled!',
                error: renderToastError,
            }),
        onSuccess: () => {
            void queryClient.invalidateQueries({ queryKey: ['decathlon'] });
    ...
    return (
        <div>
            ...
            <p>
                The rules are simple. You will be given 10 random levels which will have to be beaten in ascending
                difficulty. You cannot see all of the run ahead of time, only the next level in line.
            </p>
            ...
            {data && (
                <>
                    <p className='my-4'>
                        You have{' '}
                        <b>
                            {data.Description.reRolls} re-roll{pluralS(data.Description.reRolls)}
                        </b>{' '}
                        left
                    </p>
                            ...
                            <li className='even:bg-gray-700 py-4' key={l.LevelID}>
                                <div className='flex'>
                                    <p className='ps-2 py-1 grow'>
                                        <b>#{i + 1}</b>{' '}
                                        <Link to={`/level/${l.LevelID}`} className='underline'>
                                            {l.Level.Meta.Name}
                                        </Link>{' '}
                                        <i>
                                            <IDButton className='italic text-gray-400' id={l.LevelID} />
                                        </i>
                                    </p>
                                    <p className={`w-20 text-center py-1 tier-${l.Level.Rating!.toFixed()}`}>
                                        {l.Level.Rating!.toFixed()}
                                    </p>
                                    <p className={`w-20 text-center py-1 enj-${(l.Level.Enjoyment ?? -1).toFixed()}`}>
                                        {l.Level.Enjoyment?.toFixed() ?? 'N/A'}
                                    </p>
                                </div>
                                {i === data.Description.levelsCompleted && (
                                    <div>
                                        ...
                                        <button
                                            ...
                                            className='text-red-400 disabled:line-through disabled:text-gray-400 ms-2 underline-t'
                                            disabled={data.Description.reRolls <= -1}
                                        >
                                            Re-roll
                                        </button>
                                    </div>
