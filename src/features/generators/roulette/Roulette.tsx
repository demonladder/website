...
export default function Roulette() {
    ...
    function submit() {
        ...
        if (
            minEnjoyment.value !== '' &&
            !validateParameter(minEnjoyment.isValid, minEnjoyment.value, 'Minimum enjoyment')
        )
            return;
        if (
            maxEnjoyment.value !== '' &&
            !validateParameter(maxEnjoyment.isValid, maxEnjoyment.value, 'Maximum enjoyment')
        )
            return;
    ...
    return (
        <div>
            ...
            <p>
                Generate a roulette. For every level you have to get 1% more than the last level. Eg. you must get at
                least 1% on level #1, 2% on level #2, 3% on level #3 and so on until you reach level #100 where you have
                to get 100%.
            </p>
            <FormGroup>
                ...
                <TextInput
                    value={minTier.value}
                    onChange={(e) => minTier.setValue(e.target.value.trim())}
                    id='minTier'
                    min='1'
                    max={MAX_TIER}
                    invalid={!minTier.isValid}
                />
                <FormInputDescription>
                    Optional. The lowest tier of the roulette. Use decimals for higher precision.
                </FormInputDescription>
            </FormGroup>
            <FormGroup>
                ...
                <TextInput
                    value={maxTier.value}
                    onChange={(e) => maxTier.setValue(e.target.value.trim())}
                    id='maxTier'
                    min='1'
                    max={MAX_TIER}
                    invalid={!maxTier.isValid}
                />
                <FormInputDescription>
                    Optional. The highest tier of the roulette. Use decimals for higher precision.
                </FormInputDescription>
            </FormGroup>
            <FormGroup>
                <FormInputLabel>Minimum enjoyment</FormInputLabel>
                <TextInput
                    value={minEnjoyment.value}
                    onChange={(e) => minEnjoyment.setValue(e.target.value.trim())}
                    id='minEnjoyment'
                    min='0'
                    max='10'
                    invalid={!minEnjoyment.isValid}
                />
                <FormInputDescription>
                    Optional. The lowest enjoyment of the roulette. Use decimals for higher precision.
                </FormInputDescription>
            </FormGroup>
            <FormGroup>
                <FormInputLabel>Maximum enjoyment</FormInputLabel>
                <TextInput
                    value={maxEnjoyment.value}
                    onChange={(e) => maxEnjoyment.setValue(e.target.value.trim())}
                    id='maxEnjoyment'
                    min='0'
                    max='10'
                    invalid={!maxEnjoyment.isValid}
                />
                <FormInputDescription>
                    Optional. The highest enjoyment of the roulette. Use decimals for higher precision.
                </FormInputDescription>
            </FormGroup>
            <FormGroup>
            ...
            <PrimaryButton onClick={submit} loading={generateMutation.isPending}>
                Generate
            </PrimaryButton>
...
function List({
    ...
    return (
        <ol className='mt-4 text-xl'>
            {levels.slice(0, progress + 1).map((l, i) => (
                <li className='even:bg-gray-700 px-2 py-4' key={l.ID}>
                    <div className='flex'>
                        <p className='ps-2 py-1 grow'>
                            <b>{i + 1}%</b>{' '}
                            <Link to={`/level/${l.ID}`} className='underline'>
                                {l.Name ?? l.Meta?.Name ?? 'Unknown'}
                            </Link>{' '}
                            <i>
                                <IDButton className='italic text-gray-400' id={l.ID} />
                            </i>
                        </p>
                        <p className={`w-20 text-center py-1 tier-${l.Rating?.toFixed() ?? 0}`}>
                            {l.Rating?.toFixed() ?? '-'}
                        </p>
                        <p className={`w-20 text-center py-1 enj-${l.Enjoyment?.toFixed() ?? -1}`}>
                            {l.Enjoyment?.toFixed() ?? '-'}
                        </p>
                    </div>
                    {i === progress && (
