...
export default function RemoveList({ references, undo }: Props) {
    ...
    const referencesToChange = references
        .map((ID) => data.find((r) => r.LevelID === ID))
        ...
        <div>
            ...
            <ul>
                {referencesToChange.map((r) => (
                    <ChangeLevel
                        data={{ ID: r.LevelID, Name: r.Level.Meta.Name, Tier: r.Tier, Type: ChangeType.Remove }}
                        remove={() => undo(r.LevelID)}
                        key={r.LevelID}
                    />
                ))}
            </ul>
