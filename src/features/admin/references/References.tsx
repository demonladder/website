...
export default function EditReferences() {
    ...
    function onRemoveReference(reference: Reference) {
        ...
        setChangeList((prev) => [
            ...prev,
            { Tier: reference.Tier, ID: reference.LevelID, Type: 'remove', Name: reference.Level.Meta.Name } as Change,
        ]);
    }
    ...
    return (
        <div id='edit-references'>
            ...
            <div className='flex justify-between mb-3'>
                ...
                <PrimaryButton
                    className={changeList.length > 0 ? 'block' : 'hidden'}
                    onClick={onSave}
            ...
            <div className='flex flex-col gap-2 mb-8'>
                {data
                    ...
                    .map((l) => (
                        <Level data={l} remove={() => onRemoveReference(l)} key={l.LevelID} />
                    ))}
            </div>
            <PageButtons onPageChange={(page) => setTier(page + 1)} page={tier - 1} total={maxTier} limit={1} />
            <div className='flex justify-around'>
                <button className='text-gray-400 underline-t' onClick={() => setMaxTier((prev) => prev + 1)}>
                    Add tier
                </button>
            </div>
            {changeList.length > 0 && (
                <div className='change-list'>
                    ...
                    <div className='list'>
                        {changeList.map((c) => (
                            <ChangeLevel
                                data={c}
                                remove={() => setChangeList(changeList.filter((d) => d !== c))}
                                key={c.ID}
                            />
                        ))}
                    </div>
