...
export default function NavItem({ onClose, route, size }: Props) {
    return (
        <div className='relative group'>
            <NavLink to={route.to} onClick={onClose} className={({ isActive }) => (isActive ? 'font-bold ' : '') + 'underline'}>{route.name}{((route.subroutes ?? []).length > 0 && size === 'wide') && (<i className='bx bx-chevron-down' />)}</NavLink>
            {size === 'wide' &&
