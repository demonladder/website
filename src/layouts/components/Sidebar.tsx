...
export default function Sidebar() {
    ...
    return (
        <>
            <div
                className={
                    'fixed inset-0 bg-black z-30 fast-effect-transition ' +
                ...
                <div className='p-2 relative'>
                    ...
                    {session.user && (
                        <div>
                            {session.user?.avatar ? (
                                <img
                                    src={`https://cdn.gdladder.com/avatars/${session.user.ID}/${session.user.avatar}.png?size=${56}`}
                                    width={56}
                                    height={56}
                                ...
                                <img
                                    src={difficultyToImgSrc(
                                        session.user?.Hardest?.Meta.Difficulty,
                                        DemonLogoSizes.SMALL,
                                    )}
                                    width={56}
                                    height={56}
                                    className='mx-auto'
                    ...
                    <ul className='my-2'>
                        ...
                        {session.hasPermission(PermissionFlags.STAFF_DASHBOARD) && (
                            <MenuLink to='/mod' label='Dashboard'>
                                <Dashboard />
                            </MenuLink>
                        )}
                    </ul>
                    {session.user && (
                        <ul className='bg-theme-900 p-2 rounded-xl my-2'>
                            ...
                            <MenuLink to='/settings/profile' label='Settings'>
                                <Cog />
                            </MenuLink>
                            <button
                                onClick={onSignOut}
                    ...
                    <ul className='bg-theme-900 p-2 rounded-xl my-2'>
                        ...
                        <MenuLink to='/references' label='References'>
                            <Book />
                        </MenuLink>
                        <MenuLink to='/about' label='Guidelines'>
                            <Reading />
                        </MenuLink>
                        {session.user && (
                            <Link
                    ...
                    <ul className='bg-theme-900 p-2 rounded-xl my-2'>
                        ...
                        <MenuLink to='/generators' label='Challenges'>
                            <DiceRoll />
                        </MenuLink>
                    </ul>
                </div>
