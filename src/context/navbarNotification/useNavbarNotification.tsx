    ...
    return (
        <div className='bg-neutral-500 text-theme-text px-2 xl:px-32 py-2 min-h-8 flex justify-between items-center'>
            ...
            <div className='h-full'>
                <button
                    className='bg-button-discord-primary hover:bg-button-discord-hover active:bg-button-discord-active transition-colors px-2 h-full'
                    onClick={openModal}
                >
                    sync now
                </button>
...
export default function useNavbarNotification() {
    ...
    const discordSync = useCallback(() => {
        context.addNotification(
            'discord-sync',
            <DiscordSyncNotification onClose={() => context.removeNotification('discord-sync')} />,
        );
    }, [context]);
    ...
    return {
        info: (message: string) => {
            ...
            context.addNotification(
                ID,
                <InfoNotification message={message} onClose={() => context.removeNotification(ID)} />,
            );
        },
        warning: (message: string) => {
            ...
            context.addNotification(
                ID,
                <WarningNotification message={message} onClose={() => context.removeNotification(ID)} />,
            );
        },
        error: (message: string) => {
            ...
            context.addNotification(
                ID,
                <ErrorNotification message={message} onClose={() => context.removeNotification(ID)} />,
            );
        },
        discordSync,
    };
