...
export default function Level({
    ...
    return (
        <div
            className={
                'relative group cursor-pointer outline-offset-4 round:rounded-xl focus-visible:outline-2' +
                (selected ? ' outline-2 z-20' : '') +
                (app.enableLevelThumbnails
                    ? ''
                    : completed
                      ? ' odd:bg-green-700 even:bg-green-500'
                      : ' odd:bg-theme-700 even:bg-theme-500')
            }
            onClick={() => void navigate('/level/' + ID)}
            onContextMenu={onContextMenu}
            tabIndex={0}
        >
            {app.enableLevelThumbnails && (
                <img
                    className='absolute size-full object-cover object-center transition-all opacity-60 group-hover:opacity-100 focus-visible:opacity-100 level-thumbnail'
                    loading='lazy'
                    src={`https://levelthumbs.prevter.me/thumbnail/${IDMapper(ID)}`}
                    style={{ maskImage: 'linear-gradient(to right, transparent var(--mask-start-at, 40%), black 90%)' }}
                />
            )}
            <div className='flex justify-between h-20'>
                <div className='flex gap-4 z-10 items-center'>
                    ...
                    <div className='flex flex-col'>
                        ...
                        {position !== undefined && <p className='text-xl text-gray-400'>Queue: #{position}</p>}
                    </div>
                </div>
                <div className='flex gap-4 z-10'>
                    <div className='self-center hidden lg:block'>
                        ...
                        >
                            {ID > 3 && (
                                <p>
                                    ID:{' '}
                                    <IDButton id={ID} style={{ textShadow: '1px 2px 0px black, 0px 1px 10px black' }} />
                                </p>
                            )}
                            {songName && (
                                <p>
                    ...
                    <div className='flex text-3xl text-shadow-lg'>
                        <div className={`flex justify-center w-14 lg:w-28 tier-${roundedTier}`}>
                            <p className='group-hover:hidden self-center cursor-default'>
                                {rating !== null ? roundedTier : (defaultRating ?? 'N/A')}
                            </p>
                            <p className='hidden group-hover:block self-center cursor-default'>
                                {(actualRating ?? rating)?.toFixed(2) ?? 'N/A'}
                            </p>
                        </div>
                        <div className={`flex justify-center w-14 lg:w-28 enj-${roundedEnjoyment}`}>
                            <p className='group-hover:hidden self-center cursor-default'>
                                {enjoyment !== null ? roundedEnjoyment : 'N/A'}
                            </p>
                            <p className='hidden group-hover:block self-center cursor-default'>
                                {(actualEnjoyment ?? enjoyment)?.toFixed(2) ?? 'N/A'}
                            </p>
                        </div>
                    </div>
                </div>
