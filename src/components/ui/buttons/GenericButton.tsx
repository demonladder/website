...
export function Generic({ loading = false, children, ...props }: GenericButtonProps) {
    ...
    return (
        <button {...props} className={'relative rounded disabled:brightness-75 disabled:pointer-events-none bg-linear-to-b px-3 min-h-[1.75rem]' + (props.className ? ' ' + props.className : '')}>
