...
export default function PackRef({ pack, meta }: Props) {
    ...
    return (
        <Link
            to={`/pack/${pack.ID}`}
            className={`block relative group px-4 py-2 ${isCompleted ? 'bg-green-700' : 'bg-theme-500'} round:rounded-md border border-white/0 hover:border-white/100 transition-colors shadow-md`}
        >
            {meta && (
                <HoverMenu
