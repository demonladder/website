...
export default function PageButtons({ onPageChange, meta }: Props) {
    const [displayedValue, setDisplayedValue] = useState('' + (meta?.page || 0));

    useEffect(() => {
        setDisplayedValue('' + (meta?.page || 0));
    }, [meta?.page]);
    ...
    function onChange(e: React.ChangeEvent<HTMLInputElement>) {
        if (e.target.value === '') return setDisplayedValue('');

        const parsed = parseInt(e.target.value);
        if (isNaN(parsed)) return;

        setDisplayedValue('' + clamp(parsed, 1, maxPages));
    ...
    return (
        <div className='mt-3 flex gap-3 justify-center items-center'>
            ...
            <div className='flex items-center gap-1'>
                ...
                <NumberInput disableSpinner={true} centered={true} value={displayedValue} onChange={onChange} style={{width: '3rem'}} />
                <p>/{maxPages}</p>
            </div>
