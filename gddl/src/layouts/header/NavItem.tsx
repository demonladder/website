...
export default function NavItem({ route, size }: Props) {
    return (
        <div className='relative group'>
            <NavLink to={route.to} className={({ isActive }) => (isActive ? 'font-bold ' : '') + 'underline'}>{route.name}{(route.subroutes.length > 0 && size === 'wide') && (<i className='bx bx-chevron-down'></i>)}</NavLink>
            {size === 'wide'
                ? (
