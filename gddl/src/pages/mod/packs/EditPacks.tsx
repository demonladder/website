...
function NavButton({ to, children }: { to: string, children: React.ReactNode}) {
    return (
        <NavLink to={to} className={({ isActive }) => (isActive ? 'bg-gray-500' : 'hover:bg-gray-700') + ' text-center p-1 transition-colors select-none round:rounded-t'}>{children}</NavLink>
    );
}
...
export default function EditPacks() {
    return (
        <div id='edit-packs'>
            ...
            <div className='bg-gray-600 p-3 round:rounded-b-lg'>
                <Outlet />
            </div>
        </div>
    );
