...
import { SearchLevels, FullLevel } from '../../../api/levels';
...
export default function Search() {
    ...
    function List({ levels }: { levels: (FullLevel & {
        Completed: 0 | 1;
    })[] }) {
        return (
    ...
    function Grid({ levels }: { levels: (FullLevel & {
        Completed: 0 | 1;
        InPack: 0 | 1;
    })[] }) {
        return (
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2'>
                {levels.map((l) => <GridLevel ID={l.ID} rating={l.Rating} enjoyment={l.Enjoyment} name={l.Meta.Name} creator={l.Meta.Creator} difficulty={l.Meta.Difficulty} inPack={l.InPack === 1} key={l.ID} />)}
            </div>
        );
    }
