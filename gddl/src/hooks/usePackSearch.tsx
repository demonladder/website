...
export default function usePackSearch({ ID }: Props) {
    ...
    return {
        ...
        SearchBox: (<SearchBox<ListType> search={search} onSearchChange={setSearch} id={ID} list={data?.map((d) => ({ ...d, label: d.Name })) || []} onDelayedChange={setSearchQuery} setResult={setActivePack} status={status} invalid={isInvalid} />)
    }
}